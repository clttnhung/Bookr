...
def book_search(request):
    ...
    search_history = request.session.get('search_history', []) #truy xuat lich su
    ...
    if form.is_valid() and form.cleaned_data["search"]:
        ...
        if search_in == "title":
            ...
            for contributor in lname_contributors:
                for book in contributor.book_set.all():
                    books.add(book)
        if request.user.is_authenticated: #xem nguoi dung da xac thuc hay chua
            search_history.append([search_in, search]) #them ban ghi moi vao lich su
            request.session['search_history'] = search_history #luu doi tuong search_history vao request.session. Điều này giúp lưu trữ lịch sử tìm kiếm của người dùng cho các phiên làm việc sau này.
        elif search_history:
            initial = dict(search=search_text,
                           search_in=search_history[-1][0]) #từ điển với hai khóa: search được lấy từ biến search_text, search_in lay tu search_history
            form = SearchForm(initial=initial) #đoạn mã này tạo ra một biểu mẫu tìm kiếm mới (SearchForm) và khởi tạo nó với giá trị ban đầu từ initial. Điều này có nghĩa là giá trị tìm kiếm và vị trí tìm kiếm trước đó sẽ được điền vào biểu mẫu tìm kiếm, giúp người dùng thấy thông tin tìm kiếm trước đó khi trang web được tải lại.
